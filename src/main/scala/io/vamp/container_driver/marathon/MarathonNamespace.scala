...
trait MarathonNamespace {
  ...
  protected def appId(workflow: Workflow): String = {
    s"$nameDelimiter${namespace.id}${nameDelimiter}workflow-${artifactName2Id(workflow)}"
  }

  protected def appId(deployment: Deployment, breed: Breed): String = {
    s"$nameDelimiter${namespace.id}${nameDelimiter}deployment-${artifactName2Id(deployment)}-service-${artifactName2Id(breed)}"
  }
