...
class SqlPersistenceInitializationActor(sqlDialectInterpreter: SqlDSL ~> SqlResult) extends CommonSupportForActors with NamespaceValueResolver with LifterNotificationProvider {
  ...
  def receive = {
    case "init" ⇒
      ...
      val createUrl = Config.string("vamp.lifter.sql.connection.url")()
      val vampDatabaseUrl = resolveWithNamespace(Config.string("vamp.lifter.sql.connection.database-url")())
