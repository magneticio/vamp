...
/**
  * Initializes the database and tables for Microsoft SQL Server (if needed)
  */
class SQLServerPersistenceInitializationActor  extends CommonSupportForActors
  ...
  def receive = {
    case "init" ⇒
      ...
      Source
        .fromInputStream(getClass.getResourceAsStream("sqlserver.sql"))
        .mkString
        .split(';')
        .map(_.trim)
        .filterNot(_.isEmpty)
        .foreach(execute(dbUrl, user, password, _))
  }
  ...
  private def createDatabase(db: String, url: String, user: String, password: String) = {
    ...
    try {
      ...
      val statement = connection.createStatement()
      try {
        val result = statement.executeQuery("select * from sys.databases;")

        while (result.next()) {
          if (result.getString(1) == db) {
            dbExists = true
          }
        }
      } finally {
      ...
      if (!dbExists)
        execute(url, user, password, s"""CREATE DATABASE \"$db\";""")
    } catch {
      case e: Exception ⇒
        reportException(PersistenceOperationFailure(e))
    } finally {
      connection.close()
    }
  }
