...
class DefaultRouterDriver(ec: ExecutionContext, url: String) extends RouterDriver with DefaultRouterDriverNameMatcher {
  ...
  def all: Future[DeploymentRoutes] = {
    ...
    RestClient.get[List[Gateway]](s"$url/v1/routes").map(deploymentRoutes)
  }

  def deploymentRoutes(routes: List[Gateway]): DeploymentRoutes = {
    val clusterRoutes = routes.filter(route ⇒ processableClusterRoute(route.name)).map(route ⇒ ClusterGateway(clusterRouteNameMatcher(route.name), route.port, services(route, route.services)))
  ...
  private def create(route: Gateway, update: Boolean) = {
    if (update) {
  ...
  private def route(name: String, deployment: Deployment, cluster: Option[DeploymentCluster], port: Port) = cluster match {
    case None    ⇒ Gateway(name, port.number, if (port.`type` == Port.Http) "http" else "tcp", filters(cluster), services(deployment, None, port))
    case Some(c) ⇒ Gateway(name, c.routes.get(port.number).get, if (port.`type` == Port.Http) "http" else "tcp", filters(cluster), services(deployment, cluster, port))
  }

  private def filters(cluster: Option[DeploymentCluster]): List[gateway_driver.Filter] = {
    (cluster match {
      case None ⇒ None
      case Some(c) ⇒ c.services.flatMap { service ⇒
        service.routing.getOrElse(DefaultRouting("", None, Nil)).filters.flatMap({
          case filter: DefaultFilter ⇒ gateway_driver.Filter(if (filter.name.isEmpty) None else Option(filter.name), filter.condition, s"${artifactName2Id(service.breed, serviceIdMatcher)}") :: Nil
          case _                     ⇒ Nil
        })
      }
    }) match {
      case result: List[_] ⇒ result.asInstanceOf[List[gateway_driver.Filter]]
      case _               ⇒ Nil
    }
  }

  private def services(deployment: Deployment, cluster: Option[DeploymentCluster], port: Port): List[gateway_driver.Service] = cluster match {
    case Some(c) ⇒
      c.services.map { service ⇒ gateway_driver.Service(s"${artifactName2Id(service.breed, serviceIdMatcher)}", service.routing.getOrElse(DefaultRouting("", Some(100), Nil)).weight.getOrElse(100), service.servers.map(server(service, _, port))) }

    case None ⇒
      val name = TraitReference.referenceFor(port.name).map(_.referenceWithoutGroup).getOrElse(port.name)
      gateway_driver.Service(s"${string2Id(name, serviceIdMatcher)}", 100, servers(deployment, port)) :: Nil
  }

  private def services(route: Gateway, services: List[gateway_driver.Service]): List[GatewayService] = services.map { service ⇒
    GatewayService(serviceRouteNameMatcher(service.name), service.weight, service.servers, route.filters.filter(_.destination == service.name))
  }

  private def server(service: DeploymentService, server: DeploymentServer, port: Port) = server.ports.get(port.number) match {
    case Some(p) ⇒ gateway_driver.Server(artifactName2Id(server), server.host, p)
    case _       ⇒ gateway_driver.Server(artifactName2Id(server), server.host, 0)
  }

  private def servers(deployment: Deployment, port: Port): List[gateway_driver.Server] = {
            ...
            deployment.clusters.find(_.name == cluster) match {
              ...
              case Some(c) ⇒
                c.routes.values.find(_ == routePort.number) match {
                  case Some(_) ⇒ gateway_driver.Server(string2Id(s"${deployment.name}_${port.number}"), host.value.get, routePort.number) :: Nil
                  case _       ⇒ Nil
                }
            }
          case _ ⇒ Nil
