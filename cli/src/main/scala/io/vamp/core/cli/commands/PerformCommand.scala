...
object PerformCommand extends Parameters {
  ...
  def doCommand(command: CliCommand)(implicit vampHost: String, options: OptionMap): Unit = {
    command match {
      ...
      case _: InspectRoutingCommand =>
        println(VampHostCalls.getRouting(getParameter(name)))

      case _: InspectScaleCommand =>
        println(VampHostCalls.getScale(getParameter(name)))
