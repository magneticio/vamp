  ...
  private def requestWithCache[T](read: Boolean, operation: String, id: String, request: () ⇒ T): (String, T) = {
    cache.get[(String, Either[T, Exception])](id) match {
      case Some((op, response)) if op == operation ⇒
        ...
        response match {
          case Left(r) ⇒ {
            logger.info("K8sCache - Returning from cache for {} on operation {} response was {}", id, operation, response)
            op → r
          }
          case Right(e) ⇒ {
      ...
      case _ ⇒
        try {
          ...
          logger.info("K8sCache - Request sent for {} on operation {} response was {}", id, operation, response)
