...
import io.vamp.container_driver.kubernetes.{ Job, K8sConfig, KubernetesDriverActor }
...
class KubernetesWorkflowActor extends DaemonWorkflowDriver with ContainerDriverMapping with ContainerDriverValidation {
  ...
  override protected lazy val info: Future[Map[_, _]] = Future.successful(Map("kubernetes" → Map("url" → K8sConfig().url)))
