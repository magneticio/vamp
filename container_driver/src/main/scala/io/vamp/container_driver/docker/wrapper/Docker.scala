...
case class Docker(
  ...
  private val _auth: Option[AuthConfig] = None)(implicit ec: ExecutionContext) extends Requests(
...
object Docker {
  ...
  abstract class Completion[T: Representation](implicit ec: ExecutionContext) extends Completer {
  ...
  object Stream {
    ...
    def chunk[T: StreamRepresentation]: (T ⇒ Unit) ⇒ Docker.Handler[Unit] with Stream.Stopper = { f ⇒
      new Strings[Unit] with Docker.StreamErrorHandler[Unit] with Docker.Stream.Stopper {
