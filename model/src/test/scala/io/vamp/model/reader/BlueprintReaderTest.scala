...
class BlueprintReaderTest extends FlatSpec with Matchers with ReaderTest {
  ...
  it should "parse dialects" in {
    BlueprintReader.read(res("blueprint/blueprint47.yml")) should have(
      'name("nomadic-frostbite"),
      'clusters(List(Cluster("supersonic", List(Service(BreedReference("sava1"), Nil, None, Map(Dialect.Marathon -> Map("a" -> "b"), Dialect.Docker -> Map("c" -> "d"))), Service(BreedReference("sava2"), Nil, None, Map())), None, None, Map(Dialect.Marathon -> Map("r" -> "t"), Dialect.Docker -> Map("q" -> "w", "o" -> "p"))), Cluster("viper", List(Service(BreedReference("sava3"), Nil, None, Map()), Service(BreedReference("sava4"), Nil, None, Map())), None, None, Map(Dialect.Marathon -> Map("u" -> "i"))))),
      'endpoints(Nil),
      'environmentVariables(Nil)
    )
  }
  ...
  it should "read service environment variables" in {
    BlueprintReader.read(res("blueprint/blueprint50.yml")) should have(
      'name("nomadic-frostbite"),
      'clusters(List(Cluster("supersonic", List(Service(DefaultBreed("solid-barbershop", Deployable("docker://vamp/solid-barbershop"), Nil, List(EnvironmentVariable("HEAP", None, Some("1024MB"))), Nil, Map()), List(EnvironmentVariable("HEAP", None, Some("2GB"), None)), None, Map())), None, None, Map()))),
      'environmentVariables(Nil)
    )
  }
  ...
  it should "read service environment variables for ref breed" in {
    BlueprintReader.read(res("blueprint/blueprint51.yml")) should have(
      'name("nomadic-frostbite"),
      'clusters(List(Cluster("supersonic", List(Service(BreedReference("solid-barbershop"), List(EnvironmentVariable("HEAP", None, Some("2GB"), None)), None, Map())), None, None, Map()))),
      'environmentVariables(Nil)
    )
  }
  ...
  it should "process default port type" in {
    ...
    blueprint should have(
      ...
      'clusters(List(Cluster("supersonic", List(Service(DefaultBreed("solid-barbershop", Deployable("docker", Some("vamp/solid-barbershop")), List(Port("port", None, Some("80/http")), Port("health", None, Some("8080")), Port("metrics", None, Some("8090/tcp"))), Nil, Nil, Map()), Nil, None, Map())), None, None, Map())))
    )
