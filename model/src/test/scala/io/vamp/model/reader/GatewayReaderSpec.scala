...
class GatewayReaderSpec extends FlatSpec with Matchers with ReaderSpec {
  ...
  "GatewayReader" should "read a gateway" in {
    GatewayReader.read(res("gateway/gateway1.yml")) should have(
      ...
      'routes(List(DefaultRoute("", RootAnyMap.empty, GatewayPath("sava1", List("sava1")), Some(Percentage(50)), None, None, Nil, None), DefaultRoute("", RootAnyMap.empty, GatewayPath("sava2/v1", List("sava2", "v1")), Some(Percentage(50)), None, None, Nil, None)))
    )
  }
  ...
  it should "read a deployment gateway" in {
    GatewayReader.read(res("gateway/gateway2.yml")) should have(
      ...
      'sticky(None),
      'routes(List(DefaultRoute("", RootAnyMap.empty, GatewayPath("web/port", List("web", "port")), Some(Percentage(100)), None, None, Nil, None)))
    )
  }
  ...
  it should "read route balance" in {
    GatewayReader.read(res("gateway/gateway5.yml")) should have(
      ...
      'sticky(None),
      'routes(List(
        DefaultRoute("", RootAnyMap.empty, GatewayPath("web/port1", List("web", "port1")), Some(Percentage(40)), None, None, Nil, Some("custom 1")),
        DefaultRoute("", RootAnyMap.empty, GatewayPath("web/port2", List("web", "port2")), Some(Percentage(60)), None, None, Nil, Some("custom 2"))
      ))
    )
  }
  ...
  it should "read route rewrites" in {
    GatewayReader.read(res("gateway/gateway6.yml")) should have(
      ...
      'sticky(None),
      'routes(List(
        DefaultRoute("", RootAnyMap.empty, GatewayPath("web/port1", List("web", "port1")), None, None, None, List(PathRewrite("", "a", "b")), None),
        DefaultRoute("", RootAnyMap.empty, GatewayPath("web/port2", List("web", "port2")), Some(Percentage(100)), None, None, Nil, None)
      ))
    )
  }
  ...
  it should "parse external route" in {
    GatewayReader.read(res("gateway/gateway7.yml")) should have(
      ...
      'port(Port("8080", None, Some("8080/http"))),
      'sticky(Some(Gateway.Sticky.Route)),
      'routes(List(DefaultRoute("", RootAnyMap.empty, GatewayPath("[external/1/2]", List("[external/1/2]")), Some(Percentage(100)), None, None, Nil, None)))
    )
  }
