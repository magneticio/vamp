...
class BlueprintReaderSpec extends FlatSpec with Matchers with ReaderSpec {
  ...
  it should "parse dialects" in {
    BlueprintReader.read(res("blueprint/blueprint47.yml")) should have(
      'name("nomadic-frostbite"),
      'clusters(List(Cluster("supersonic", Map(), List(Service(BreedReference("sava1"), Nil, None, Nil, None, None, Map(Dialect.Marathon → Map("a" → "b"), Dialect.Docker → Map("c" → "d"))), Service(BreedReference("sava2"), Nil, None, Nil, None, None, Map())), Nil, None, None, None, Map(Dialect.Marathon → Map("r" → "t"), Dialect.Docker → Map("q" → "w", "o" → "p"))), Cluster("viper", Map(), List(Service(BreedReference("sava3"), Nil, None, Nil, None, None, Map()), Service(BreedReference("sava4"), Nil, None, Nil, None, None, Map())), Nil, None, None, None, Map(Dialect.Marathon → Map("u" → "i"))))),
      'gateways(Nil),
      'environmentVariables(Nil)
    )
  }
