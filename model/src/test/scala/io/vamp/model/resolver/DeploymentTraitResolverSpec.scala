...
class DeploymentTraitResolverSpec extends FlatSpec with Matchers with DeploymentTraitResolver with ModelNotificationProvider {
  ...
  it should "pass through environment variables for non relevant clusters" in {
    ...
    resolveEnvironmentVariables(deployment(environmentVariables), DeploymentCluster("backend", Map(), Nil, Nil, None, None) :: Nil).filter(ev ⇒ filter.contains(ev.name)) should equal(
      environmentVariables
    )
  }
  ...
  def deployment(environmentVariables: List[EnvironmentVariable]) = {
    val clusters = DeploymentCluster("backend1", Map(), Nil, Nil, None, None) :: DeploymentCluster("backend2", Map(), Nil, Nil, None, None) :: Nil
