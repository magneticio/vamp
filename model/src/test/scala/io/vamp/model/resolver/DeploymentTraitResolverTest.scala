...
class DeploymentTraitResolverTest extends FlatSpec with Matchers with DeploymentTraitResolver {
  ...
  it should "pass through environment variables for non relevant clusters" in {
    ...
    resolveEnvironmentVariables(deployment(environmentVariables), DeploymentCluster("backend", Nil, Nil, None) :: Nil).filter(ev ⇒ filter.contains(ev.name)) should equal(
      environmentVariables
    )
  }
  ...
  def deployment(environmentVariables: List[EnvironmentVariable]) = {
    val clusters = DeploymentCluster("backend1", Nil, Nil, None) :: DeploymentCluster("backend2", Nil, Nil, None) :: Nil
