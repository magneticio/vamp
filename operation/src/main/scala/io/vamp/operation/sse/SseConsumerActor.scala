...
class SseConsumerActor extends Percolator with CommonSupportForActors with OperationNotificationProvider {
  ...
  private val streamUrl = context.system.settings.config.getString("vamp.rest-api.sse.router-stream")
