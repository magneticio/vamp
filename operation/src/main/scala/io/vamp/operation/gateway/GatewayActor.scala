...
class GatewayActor extends ArtifactPaginationSupport with CommonSupportForActors with OperationNotificationProvider with GatewayRouteValidation with LazyLogging with RouteComparator {
  ...
  private def update(gateway: Gateway, source: Option[String], validateOnly: Boolean, force: Boolean): Future[Any] = {
    ...
    def default = {
      if (validateOnly)
        Try((process(false) andThen validate)(gateway)).recover({ case e ⇒ Future.failed(e) }).map(Future.successful).get
      else
  ...
  private def process : Gateway => Gateway = {
    logger.info("process without args")
    process(true)
  }

  private def process(generateEvents: Boolean): Gateway ⇒ Gateway = { gateway ⇒

    logger.info("process: " + generateEvents)
