...
trait GatewayApiController extends AbstractController {
  ...
  protected def createGateway(artifact: Artifact, source: String, validateOnly: Boolean)(implicit namespace: Namespace, timeout: Timeout) = {
  ...
  protected def updateGateway(artifact: Artifact, name: String, source: String, validateOnly: Boolean)(implicit namespace: Namespace, timeout: Timeout) = {
    expandGateway(artifact.asInstanceOf[Gateway]) flatMap { gateway ⇒
