...
import io.vamp.core.operation.deployment.{DeploymentActor, DeploymentSchedulerActor, DeploymentSynchronizationActor}
...
object OperationBootstrap extends Bootstrap {
  ...
  def run(implicit actorSystem: ActorSystem) = {
    ...
    ActorSupport.actorOf(DeploymentSchedulerActor) ! SchedulerActor.Period(ConfigFactory.load().getInt("deployment.synchronization.period"))
