...
import io.vamp.persistence.{KeyValueStoreActor, KeyValueStorePath, PersistenceMarshaller}
...
object GatewayDriverActor {
  ...
  val rootPath = GatewayAccessPath("gateways" :: Nil)
...
class GatewayDriverActor(marshallers: Map[String, GatewayMarshallerDefinition]) extends PersistenceMarshaller with PulseFailureNotifier with CommonSupportForActors with GatewayDriverNotificationProvider {
  ...
  private def get(path: GatewayAccessPath): Future[Option[String]] = {
    IoC.actorFor[KeyValueStoreActor] ? KeyValueStoreActor.Get(path.toKeyValuePath) map {
      case Some(content: String) ⇒ Option(content)
      case _                     ⇒ None
    }
  }

  private def set(path: GatewayAccessPath, value: String): Future[_] = {
    IoC.actorFor[KeyValueStoreActor] ? KeyValueStoreActor.Get(path.toKeyValuePath) flatMap {
      case Some(content: String) if value == content ⇒ Future.successful(None)
      case _                                         ⇒ IoC.actorFor[KeyValueStoreActor] ? KeyValueStoreActor.Set(path.toKeyValuePath, Some(value))
    }
  }

  private def pull(): Future[List[Gateway]] = {
    IoC.actorFor[KeyValueStoreActor] ? KeyValueStoreActor.Get(rootPath.toKeyValuePath) map {
...
case class GatewayAccessPath(path: List[String]) {
  def toKeyValuePath: KeyValueStorePath = KeyValueStorePath(path)
  def :+(pathElement: String) = GatewayAccessPath (this.path :+ pathElement)
}