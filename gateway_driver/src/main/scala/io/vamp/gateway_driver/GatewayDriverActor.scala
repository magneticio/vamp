...
import io.vamp.gateway_driver.GatewayStore.Put
...
trait GatewayConverter extends GatewayNameMatcher {
...
//    def `type`(port: Port): String = if (port.`type` == Port.Type.Http) "http" else "tcp"
//    cluster match {
