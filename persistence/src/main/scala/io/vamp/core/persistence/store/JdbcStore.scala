 ...
 */
class JdbcStore(executionContext: ExecutionContext)
  extends Store
  with ScaleStore
  with PortStore
  ...
  with SlaStore with EscalationStore with ParameterStore with PersistenceNotificationProvider {
  ...
  private val logger = Logger(LoggerFactory.getLogger(classOf[JdbcStore]))
