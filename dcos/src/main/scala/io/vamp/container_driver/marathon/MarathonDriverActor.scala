...
class MarathonDriverActor
  ...
  private def requestPayload(deployment: Deployment, cluster: DeploymentCluster, service: DeploymentService, app: MarathonApp): JValue = {
    val (local, dialect) = (deployment.dialects.get(MarathonDriverActor.dialect), cluster.dialects.rootMap.get(MarathonDriverActor.dialect), service.dialects.rootMap.get(MarathonDriverActor.dialect)) match {
